...
public class FingerLabeller implements IrCameraListener {
	...
	public void irImageChanged(IrCameraEvent evt) {
		...
		boolean isIThumb = d(in[maxI], 
				in[perFingerMin[maxI]]) > 
		d(in[maxJ], 
		...
		IrCameraEvent event = new IrCameraEvent(evt.getSource(), evt.getMode(), in[thumb], in[perFingerMin[thumb]], in[perFingerMin[pinky]], in[pinky]);
		for (IrCameraListener l : listeners) l.irImageChanged(event);
	}
