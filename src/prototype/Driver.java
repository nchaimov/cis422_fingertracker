...
public class Driver {
	...
	public static void main(String[] arrrgs) throws InterruptedException {
		...
		/*
		 * JFrame frame = new JFrame("WiiMote Point Tracker"); SwingPointTracker
		 * tracker = new SwingPointTracker(); frame.add(tracker); Dimension size
		 * = new Dimension(1024,768); frame.setSize(size);
		 * frame.setPreferredSize(size); frame.setMinimumSize(size);
		 ...
		 */IrCameraListener cameraListener = new IrCameraListener() {
			public void irImageChanged(IrCameraEvent evt) {
				// System.out.println("X = " + evt.getIrPoint(0).x + ", Y = " +
				// evt.getIrPoint(0).y);
